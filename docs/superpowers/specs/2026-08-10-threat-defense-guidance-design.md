...
Coverage is a dedicated visual mode rather than another annotation layer. The ordinary light/dark pattern disappears while the lens is open; the wooden frame, a uniform low-contrast 8×8 grid, and the chess pieces preserve the object's identity as a chessboard. The grid draws all seven internal horizontal and seven internal vertical boundaries regardless of neighboring coverage states, so its structure never changes as pieces move. It is one board-wide overlay rather than 64 per-square strokes, preventing shared edges from becoming darker. The initial treatment uses `AppTheme.boardFrame` at `0.18` opacity with a `0.75`-point stroke; both values remain view-style constants for real-iPad tuning. The grid appears only during coverage, sits above every square surface and selection highlight but below trajectories and pieces, ignores input, and adds no separate accessibility element.

At rest, pieces recede slightly, coordinate labels and coordinate emphasis are hidden, and trajectories, danger bursts, shields, and supporter echoes are suppressed. This lets the player read the board as a small number of connected visual regions instead of dozens of independent marks. Exact opaque surface colors, grid contrast, piece recession, and transition values remain view-style constants that should be tuned on the real iPad board. Per-square surface opacity or light/dark modulation is not allowed because it would recreate the ambiguity this treatment is intended to remove.
...
| Coverage square boundaries | Uniform, low-contrast board-wide grid |
