...
class PullRequestCIConfigurationTests(unittest.TestCase):
    def test_workflow_runs_the_approved_read_only_test_contract(self):
        ...
        self.assertRegex(
            ...
            r"python -m unittest discover -s CoachingServer/tests -v 2>&1 \| "
            r"tee python-test-output\.txt.*?"
            r"python -m unittest discover -s Tools/CoachingEval/tests -v 2>&1 \| "
            r"tee -a python-test-output\.txt.*?"
